...
class Fiber(desc.ResetMixin):
        ...
        for this_grad in grad.T:
            ...
            this_Q = (np.matrix(v) *
                 np.matrix(np.diag([axial_diffusivity,
                                    ...
                                    radial_diffusivity])) *
                np.matrix(v).T)
            tensors.append(mtt.Tensor(this_Q))
        return tensors
    ...
    def predicted_signal(self,
        ...
        ----------
        """
        tensor_list = self.tensors(axial_diffusivity, radial_diffusivity)
        S = []
        ...
        if np.iterable(S0) and len(S0) == len(tensor_list):
            for idx, T in enumerate(tensor_list):
                S.append(T.predicted_signal(S0[idx], bvecs, bvals))

        # Or just one global S0:
        else: 
            for idx, T in enumerate(tensor_list):
                S.append(T.predicted_signal(S0, bvecs, bvals))
