...
def predict_n(data, bvals, bvecs, mask, n, vox, b_mode):
    ...
    for bi in np.arange(len(unique_b[1:])):
        ...
        for combo_num in np.arange(np.floor(100./n)):
            ...
            vec_combo = these_b_inds[vec_pool[(combo_num)*num_choose:(combo_num*num_choose + num_choose)]]
            vec_combo_rm0 = these_b_inds_rm0[vec_pool[(combo_num)*num_choose:(combo_num*num_choose + num_choose)]]
               
            # Remove the chosen indices from the rest of the indices
            for choice_idx in np.array(vec_combo):
                these_inc0.remove(choice_idx)
            ...
            if b_mode is "all":
                mod = sfm_mb.SparseDeconvolutionModelMultiB(this_data, these_bvecs, these_bvals, mask = mask,
                                                           params_file = "temp")
                predicted[vec_combo_rm0] = mod.predict(bvecs[:, vec_combo], bvals[vec_combo])[vox]
            elif b_mode is "bvals":
                mod = sfm.SparseDeconvolutionModel(this_data, these_bvecs, these_bvals, mask = mask,
                                                       params_file = "temp")
...
def predict_bvals(data, bvals, bvecs, mask, b_fit_to, b_predict):
    ...
    mod = sfm.SparseDeconvolutionModel(data[:,:,:,all_inc_0], bvecs[:,all_inc_0],
                                               rounded_bvals[all_inc_0], mask = mask, params_file = 'temp')
