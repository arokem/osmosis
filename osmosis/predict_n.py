...
def new_mean_combos(vec_pool_inds, data, bvals, bvecs, mask, b_inds,
                    bounds="preset", mean_mod_func = "bi_exp_rs",
...
def _predict_across_b(mod_obj, vec_combo, vec_pool_inds, bvecs, bvals,
    ...
    """
    predicted_across = mod_obj.predict(
                                  bvecs[:, b_inds[1:][b_across][vec_pool_inds]],
...
def _preload_regressors(si, full_mod_obj, mod_obj, over_sample, mean):
    ...
    if over_sample is None:
        # Take the reduced vectors from both the b vectors and
        # rotational vectors
    ...
    if mean == "mean_model":
        ...
        return [fit_to, tensor_regressor, fit_to_demeaned,
                fit_to_means, design_matrix]

def _kfold_xval_setup(bvals, mask):
    """
    Helper function to help set up any separation of b values and initial
    reallocating
...
def _aggregate_fODFs(all_mp_list, all_mp_rot_vecs_list, unique_b, precision,
    ...
    ----------
    all_mp_list: list
        A list containing all the model parameters for calculating the
        reliability from
    all_mp_rot_vecs_list: list
        A list containing all the rotational vectors corresponding to the model
        parameters.
    unique_b: 1 dimensional array
        All the unique b values including zero
    precision_type: str
        emd: Computes the EMD between different fODFs
        emd_multi_combine: Computes the EMD for the multi fODF between model
                           parameters from all b-values combined.
        sph_cc:  Computes the spherical cross-correlation between different
                 model parameters
    start_fODF_mode: str
        The fODF mode for this round of k-fold cross-validation

    Returns
    -------
    out_mp_list: list
        Rearranged model parameters list
    out_rot_vecs_list: list
        Rearranged rotational vectors list
    """
...
def _calc_precision(mp1, mp2, rot_vecs1, rot_vecs2, idx1, idx2, mp_count, vox,
    ...
    Parameters
    ----------
    mp1: 2 dimensional array
        First set of model parameters
    mp2: 2 dimensional array
        Second set of model parameters
    rot_vecs1: 2 dimensional array
        All the rotational vectors to which the weights in the first set of
        model parameters correspond to
    rot_vecs2: 2 dimensional array
        All the rotational vectors to which the weights in the second set of
        model parameters correspond to
    idx1: int
        Index pointing to the current model parameter in mp1
    idx2: int
        Index pointing to the current model parameter in mp2
    mp_count: int
        Current combination index of model parameters
    vox: int
        Current voxel
    p_arr: 1 dimensional array
        The current, aggregate, precision array
    precision_type: str
        emd: Computes the EMD between different fODFs
        emd_multi_combine: Computes the EMD for the multi fODF between model
                           parameters from all b-values combined.
...
def kfold_xval(data, bvals, bvecs, mask, ad, rd, n, fODF_mode,
        ...
        for combo_num in np.arange(np.floor(100./n)):
            ...
            if b_idx2 == None:
                ...
                if ((mean == "MD") & (fODF_mode == "multi")):
                    mod.regressors
                #Use empirical regressors but mean model
            ...
            if precision is False:
                if b_idx2 != None:
                    # Since we're using a separate output for each prediction,
                    # and not indexing into one big array, use the indices
                    # starting from 0:
                    vec_combo_rm0 = vec_pool_inds
                    if bi == b_idx1:
                        predicted12[:, vec_pool_inds] = mod.predict(bvecs[:,
                        b_inds[1:][b_idx2][vec_pool_inds]],
                        bvals[b_inds[1:][b_idx2][vec_pool_inds]],
                        new_params = new_params)[mod.mask]
                        predicted11[:, vec_combo_rm0] = mod.predict(
                                        bvecs[:, vec_combo], bvals[vec_combo],
                                              new_params = new_params)[mod.mask]
                    else:
                        predicted21[:, vec_pool_inds] = mod.predict(bvecs[:,
                        b_inds[1:][b_idx1][vec_pool_inds]],
                        bvals[b_inds[1:][b_idx1][vec_pool_inds]],
                        new_params = new_params)[mod.mask]
                        predicted22[:, vec_combo_rm0] = mod.predict(
                                              bvecs[:, vec_combo],
                                              bvals[vec_combo],
                                              new_params = new_params)[mod.mask]
                else:
                ...
                #Save both the model params and their corresponding rotational
                # vectors for precision function
    ...
    if ((start_fODF_mode[:4] == "both") & (start_fODF_mode != "both_s") |
        ...
        [mp_list, mp_rot_vecs_list] = _aggregate_fODFs(all_mp_list,
                                                       all_mp_rot_vecs_list,
                                                       unique_b, precision,
                                                       start_fODF_mode)
        p_arr = kfold_xval_precision(mp_list, mod.mask, mp_rot_vecs_list,
        ...
        p_arr = kfold_xval_precision(all_mp_list, mod.mask,
                               all_mp_rot_vecs_list, precision, start_fODF_mode)
        p_list.append(p_arr)
...
def kfold_xval_precision(mp_list, mask, rot_vecs_list,
                        precision_type, start_fODF_mode):
    """
    Helper function that finds the spherical cross-correlation between the
    different model params generated by k-fold cross-validation.
    ...
    cc_arr: 2 dimensional array
        An array consisting of the spherical cross-correlations between each of
        the combinations of model parameters at each voxel
    """
...
def predict_grid(data, bvals, bvecs, mask, ad, rd, n, over_sample=None,
                 solver=None, mean="mean_model", fit_method=None,
                 bounds="preset"):
    """
    ...
    for bi in np.arange(3):
        ...
        for ub2 in np.arange(len(group_list)):
            ...
            these_b_inds_rm0 = np.concatenate((these_b_inds_rm0,
                                               b_inds_rm0[ub2][this_group]))
        ...
        for combo_num in np.arange(np.floor(100./n)):
            ...
            mod = sfm.SparseDeconvolutionModelMultiB(this_data, these_bvecs,
                                                     ...
                                                     over_sample = over_sample,
                                                     solver = solver)
            sig_out, new_params, inds_arr = new_mean_combos(vec_pool_inds, data,
                                                        bvals, bvecs, mask,
                                                        b_inds, bounds=bounds,
                                                       these_b_inds=these_b_inds)
...
def demean(fit_to, tensor_regressor, mod):
    ...
    ----------
    fit_to: 2 dimensional array
        Original signal fitted to.  Size should be equal to the number of voxels
        by the number of directions.
    tensor_regressor: 2 dimensional array
        The predicted signals from the tensor model.  Size should be equal to
        the number of directions fit to by the number of directions fit to.
    ...
    for bidx in np.arange(len(mod.unique_b)):
        for vox in xrange(mod._n_vox):
            ...
            fit_to_demeaned[vox, mod.b_inds_rm0[bidx]] = (fit_to[vox,
                                                          mod.b_inds_rm0[bidx]]
                                                        - np.mean(fit_to[vox,
                                                        mod.b_inds_rm0[bidx]]))
            fit_to_means[vox, mod.b_inds_rm0[bidx]] = np.mean(fit_to[vox,
                                                          mod.b_inds_rm0[bidx]])
...
def kfold_xval_gen(model_class, data, bvecs, bvals, k, mask = None,
                   fODF_mode = "single", **kwargs):
    """
    ...
    for bi in indices:
        ...
        for combo_num in np.arange(np.floor(100./k)):
            (si, vec_combo, vec_combo_rm0,
            vec_pool_inds, these_bvecs, these_bvals,
            this_data, these_inc0) = ozu.create_combos(bvecs, bvals_pool, data,
                                                       these_b_inds,
                                                       these_b_inds_rm0,
                                                       all_inc_0, vec_pool,
                                                       num_choose, combo_num)
            ...
            # If the number of inputs is greater than two, then the
            # b values are needed
            if len(inspect.getargspec(mod.predict)[0]) > 2:
                predicted[:, vec_combo_rm0] = mod.predict(bvecs[:, vec_combo],
                                              bvals[vec_combo])[mod.mask]
            # If the number of inputs is equal to two, then the b values
            # are not needed
            elif len(inspect.getargspec(mod.predict)[0]) == 2:
                predicted[:, vec_combo_rm0] = mod.predict(bvecs[:,
                                                          vec_combo])[mod.mask]
...
def predict_bvals(data, bvals, bvecs, mask, ad, rd, b_idx1, b_idx2, n = 10,
    ...
    """
    bval_list, b_inds, unique_b, rounded_bvals = separate_bvals(bvals)
    bval_list_rm0, b_inds_rm0, unique_b_rm0, rounded_bvals_rm0 = separate_bvals(
                                                        bvals, mode = 'remove0')
    ...
    if mode == "kfold_xval":
        ...
        predicted12, predicted22, predicted21] = kfold_xval(data, bvals, bvecs,
                                                            mask, ad, rd, n,
                                                            fODF_mode = "multi",
                                                            b_idx1 = b_idx1,
                                                            b_idx2 = b_idx2,
        ...
        mod = sfm.SparseDeconvolutionModelMultiB(data[:,:,:,all_inc_0],
                                                    ...
                                                    solver = solver,
                                                    params_file = 'temp')
        predicted11 = mod.predict(bvecs[:, predict_inds],
                                  bvals[predict_inds])[mod.mask]
        actual1 = actual
...
def nchoosek(n,k):
    """
    Finds all the number of unique combinations from choosing groups of
    k from a pool of n.
...
def fODF_EMD(fODF1, fODF2, bvecs1=None, bvecs2=None, dist=None):
    ...
    # degrees:
    my_emd = emd.emd(fODF1/np.sum(fODF1),
                     fODF2/np.sum(fODF2), dist) / (np.pi/2)
