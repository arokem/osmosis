...
def place_files(file_names, expected_file_num, mask_data, file_path=os.getcwd(),
                vol=False, f_type="npy", save=False):
    """
            ...
            if this_file[(len(this_file)-len(f_type)):len(this_file)] == f_type:
                ...
                if this_file[0:len(fn)] == fn:
                    if count == 0:
                        ...
                        if vol is False:
                            aggre = np.squeeze(np.zeros((int(np.sum(mask_data))
                                                             ,) + (num_dirs,)))
                        else:
                    ...
                    low = i*sub_data.shape[0]
                    high = np.min([(i+1) * sub_data.shape[0],
