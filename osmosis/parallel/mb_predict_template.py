...
if __name__=="__main__":
    ...
    actual_pn_grid, predicted_pn_grid = pn.predict_grid(data, bvals, bvecs,
                                                       ...
                                                       over_sample = 362, solver = "nnls")
    aff = np.eye(4)
    nib.Nifti1Image(predicted_pn_all, aff).to_filename("/hsgs/nobackup/klchan13/oversample_all_predict%s.nii.gz"%(i))
    nib.Nifti1Image(predicted_pn_bvals, aff).to_filename("/hsgs/nobackup/klchan13/oversample_bvals_predict%s.nii.gz"%(i))
    nib.Nifti1Image(predicted_pn_grid, aff).to_filename("/hsgs/nobackup/klchan13/oversample_grid_predict%s.nii.gz"%(i))
